...
class BaseDataset(Dataset):
	...
	def create_X(self, mol, num_points): 
		mol = self.ensemble_mol(mol, conformer='etkdg') # generate the conformers
...
class ChiralityDataset(BaseDataset): 
	...
	def __getitem__(self, idx): 
		mol = self.supp[idx]
		smiles = Chem.MolToSmiles(mol, isomericSmiles=False) # convert to regular SMILES
		isomers = tuple(EnumerateStereoisomers(Chem.MolFromSmiles(smiles))) # get all isomers
		# assert len(isomers) >= 2, 'Only {} isomers are found!'.format(len(isomers))
		# m1, m2 = random.choices(isomers, k=2)
		if len(isomers) < 2: 
			m1 = mol
			m2 = Chem.Mol(mol)
			# print('Only one isomer is found!')
		else: 
			m1, m2 = random.choices(isomers, k=2)
		
		smiles1 = Chem.MolToSmiles(m1, isomericSmiles=True)
		smiles2 = Chem.MolToSmiles(m2, isomericSmiles=True)
		
		X1, mask1 = self.create_X(m1, self.num_points)
		X2, mask2 = self.create_X(m2, self.num_points)
		...
		if self.num_csp > 1: 
			...
			multi_Y[mb] = Y
			return smiles1, smiles2, mb, X1, X2, mask1, torch.Tensor(multi_Y)
		else: 
			return smiles1, smiles2, mb, X1, X2, mask1, Y
