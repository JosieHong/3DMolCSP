...
class MolNet(nn.Module): 
	def __init__(self, args, device): 
		...
		self.encoder1 = Encoder(in_dim=args['in_channels'], 
								layers=args['encoder_layers'], 
									emb_dim=args['emb_dim'], 
									k=args['k'], 
									device=device)
		self.encoder2 = Encoder(in_dim=args['in_channels'], 
								layers=args['encoder_layers'], 
									emb_dim=args['emb_dim'], 
									k=args['k'], 
									device=device)
		
		self.csp_num = args['csp_num']
	...
	def forward(self, x1: torch.Tensor, 
		...
		'''
		Input: 
			x1:		point set, torch.Size([batch_size, 21, num_atoms])
			x2:		point set, torch.Size([batch_size, 21, num_atoms])
			env:    experimental condiction
			idx_base:   idx for local knn
		'''
		assert x1.size() == x2.size(), '{} != {}'.format(x1.size(), x2.size())
		batch_size = x1.size(0)
		
		x1 = self.encoder1(x1, torch.clone(idx_base)) # torch.Size([batch_size, emb_dim])
		x2 = self.encoder2(x2, torch.clone(idx_base)) # torch.Size([batch_size, emb_dim])
		x = x1 + x2
